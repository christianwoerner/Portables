...
<xsl:stylesheet version="1.0"
  ...
  <xsl:key name="keySRS" match="//wfs:FeatureType/wfs:OtherSRS|//wfs:FeatureType/wfs:DefaultSRS|//wfs:FeatureType/wfs:OtherCRS|//wfs:FeatureType/wfs:DefaultCRS" use="."></xsl:key>
  ...
  <xsl:template match="/">
      ...
      <body>
        ...
        <table class="statistics">
          ...
          <tr>
            ...
            <td>
              <xsl:for-each select="//wfs:FeatureType/wfs:DefaultSRS[generate-id() = generate-id(key('keySRS', .)[1])]">
                <xsl:value-of select="."/>
                (Default)<br/>
              </xsl:for-each>
              <xsl:for-each select="//wfs:FeatureType/wfs:OtherSRS[generate-id() = generate-id(key('keySRS', .)[1])]">
                <xsl:value-of select="."/>
                <br/>
              </xsl:for-each>
              <xsl:for-each select="//wfs:FeatureType/wfs:DefaultCRS[generate-id() = generate-id(key('keySRS', .)[1])]">
                <xsl:value-of select="."/>
                (Default)<br/>
              </xsl:for-each>
              <xsl:for-each select="//wfs:FeatureType/wfs:OtherCRS[generate-id() = generate-id(key('keySRS', .)[1])]">
                <xsl:value-of select="."/>
                <br/>
              </xsl:for-each>
            </td>
    ...
    <table class="service">
      ...
      <tr>
        ...
        <td>
          <xsl:if test="count(ows:ServiceIdentification/ows:ServiceType) != 0">
            <xsl:value-of select="ows:ServiceIdentification/ows:ServiceType"/>
          </xsl:if>
          <xsl:if test="count(ows:ServiceIdentification/ows:ServiceTypeVersion) != 0">
            <xsl:text> - </xsl:text>
